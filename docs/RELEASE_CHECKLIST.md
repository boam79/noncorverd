...
## 1. 환경 및 설정
- [ ] `docs/ENVIRONMENT_MATRIX.md` 검토, 누락된 환경변수 없음
- [ ] Vercel / EC2 환경변수 최신화 완료
- [ ] Secrets Manager, GitHub Secrets 검증 완료
- [ ] PM2 환경(`pm2 env`) 값 확인 및 백업

## 2. 테스트 및 검증
- [ ] `QA_CHECKLIST.md` 스테이징 환경 기준 통과
- [ ] Playwright E2E (스테이징 URL) 통과
- [ ] `scripts/load-test.js` 실행 결과 SLA 충족 (평균 < 2초, 실패 0건)
- [ ] API 로그 검토 (Vercel, PM2, CloudWatch)

## 3. 장애/롤백 대응
- [ ] 백엔드 다운 시 사용자 메시지 확인 (프론트엔드 에러 처리)
- [ ] 공공데이터 API 401/500 Mock 테스트 (백엔드 로그 및 응답 확인)
- [ ] PM2 롤백 절차 실습 (pm2 save/restore)
...
## 5. 배포 실행 전
- [ ] GitHub Actions 워크플로우 점검 (`deploy.yml`, `test.yml`)
...
> 모든 항목이 체크된 후 Planner에게 최종 Go/No-Go 결정을 요청하세요. No-Go 시에는 롤백 절차와 후속 조치를 문서화해야 합니다.
